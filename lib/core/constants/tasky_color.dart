import 'package:flutter/material.dart';

class TaskyColor {





  static const Color black1 = Color(0xFF000000);
  static const Color black2 = Color(0xFF030A0B);
  static const Color gray0 = Color(0xFF282828);
  static const Color grayText = Color(0xFF8D8D8D);
  static const Color grayText1 = Color(0xFF848A94);
  static const Color grayText2 = Color(0xFF848A94);
  static const Color gray1 = Color(0xFF747474);

  static const Color gray_2 = Color(0xFF868D95);
  static const Color gray3 = Color(0xFFCCCCCC);
  static const Color gray4 = Color(0xFFD7D7D7);
  static const Color gray5 = Color(0xFFDFDFDF);
  static const Color waiteBG = Color(0xFFF8F7F7);
  static const Color white = Color(0xFFFFFFFF);
  static const Color darkBlue = Color(0xFF002055);
  static const Color purple = Color(0xFF756EF3);
  static const Color blue = Color(0xFF63B4FF);
  static const Color blue2 = Color(0xFFC5DAFD);
  static const Color blue3 = Color(0xFFD8ECFF);
  static const Color red = Color(0xFFFF0000);
  static const Color orange = Color(0xFFFF8500);
  static const Color orangeContainer = Color(0xFFFFB35A);
  static const Color lightOrange = Color(0xFFFFE1AC);
  static const Color lightOrange2 = Color(0xFFFFD5A7);
  static const Color lightOrange3 = Color(0xFFFFC07B);
  static const Color lightOrange4 = Color(0x35850036);
  static const Color green = Color(0xFFB1D199);
  static const Color lightGreen = Color(0xFFB2D29D);
}
